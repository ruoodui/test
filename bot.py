...
# ======= تحميل بيانات الأسعار (محدثة لتشمل الرام والذاكرة) =======
...
# ======= أزرار القائمة الرئيسية (حذف خيار البحث عن الماركة) =======
def main_menu_keyboard():
    keyboard = [
        [InlineKeyboardButton("🔤 البحث عن طريق الاسم", callback_data="search_by_name")],
        # تم حذف خيار البحث عن الماركة
...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "search_by_store":
        ...
        await query.edit_message_text(f"✏️ الآن أرسل نص البحث لـ اسم الجهاز:")
        context.user_data['search_mode'] = "name"
        context.user_data['search_results'] = []
    elif data == "search_by_price":
        await query.edit_message_text(f"✏️ الآن أرسل السعر للبحث:")
        context.user_data['search_mode'] = "price"
        context.user_data['search_results'] = []
    else:
        await query.edit_message_text("⚠️ خيار غير معروف.", reply_markup=back_to_menu_keyboard())
    ...
    if mode == "price":
        ...
        for name in results[:10]:
            for spec in price_data[name]:
                ...
                msg = (
                    f"📱 {name}\n"
                    f"💾 الرام والذاكرة: {spec.get('ram_memory', 'غير متوفر')}\n"
                    f"💰 السعر: {spec['price']}\n"
                    f"🏬 المتجر: {spec['store']}\n"
...
# ======= عرض تفاصيل الجهاز (محدثة لتشمل الرام والذاكرة والماركة) =======
async def device_option_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for spec in price_data[device_name]:
        brand = device_name.split()[0] if device_name else "غير متوفر"
        msg += (
            f"📱 <b>{device_name}</b>\n"
            f"💾 الرام والذاكرة: {spec.get('ram_memory', 'غير متوفر')}\n"
            f"💰 السعر: {spec['price']}\n"
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(brand_store_selected_callback, pattern="^store_"))
